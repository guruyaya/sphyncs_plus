...
class SphincsTreeSecret(SphincsTreePublic):
  ...
  def sign(self, message: bytes, key_num: int) -> SphincsTreeProof:
    if key_num >= 2 ** self.num_levels:
      raise Exception("Key number is out of range")

    pair_num_pos = []

    wots_key = self.base_level[key_num // 2][key_num % 2]
    signature = wots_key.sign(message)
    ...
    return SphincsTreeProof(message_hash=message_hash, signature=signature, proof_tree=pair_num_pos, public_key=self.public_key)
