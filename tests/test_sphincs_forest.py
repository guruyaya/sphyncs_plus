...
def forest_secret(pytestconfig: pytest.Config) -> tuple[SphincsForestSecretGenerator, SphincsForestSecretKey]:
    ...
    if secret is None:
        secret = SphincsForestSecretGenerator(SHA256Hasher(), CSPRNGKeyGenerator(), 123)
    if pk is None:
        pk = secret.get_secret_key(8)
    return secret, pk

@pytest.fixture
def forest_secret_modified(pytestconfig: pytest.Config) -> tuple[SphincsForestSecretGenerator, SphincsForestSecretKey]:
    secret = pytestconfig.cache.get("secret_generator", None)
    pk = pytestconfig.cache.get("secret_key", None)
    if secret is None:
        secret = SphincsForestSecretGenerator(SHA256Hasher(), CSPRNGKeyGenerator(), 123, 16, 4)
    if pk is None:
        pk = secret.get_secret_key(20)
    return secret, pk


def test_signature(forest_secret: tuple[SphincsForestSecretGenerator, SphincsForestSecretKey]):
    ...
    assert signature.signature.message_hash.hex() == "a591a6d40bf420404a011733cfb7b190d62c65bf0bcda32b57b277d9ad9f146e"
...
def test_signature_for_modified(forest_secret_modified: tuple[SphincsForestSecretGenerator, SphincsForestSecretKey]):
...
def test_forest_key_generation(forest_secret: tuple[SphincsForestSecretGenerator, SphincsForestSecretKey]):
    ...
    secret = SphincsForestSecretGenerator(SHA256Hasher(), CSPRNGKeyGenerator(), 124)
    pk = secret.get_secret_key(8)
...
def test_forest_modefied_structure_keygen(forest_secret_modified: tuple[SphincsForestSecretGenerator, SphincsForestSecretKey]):
    # this is for local testing purpose
    secret, pk = forest_secret_modified
